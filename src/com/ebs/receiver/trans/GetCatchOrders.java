...
public class GetCatchOrders {
	private static Logger logger = Logger.getLogger(GetCatchOrders.class);
	public String work(String msg) {
		try{
         ...
         List<OrderInfo>list=new CatchOrderService().getOrderInfoList(lott,pageInfo);  
        if(list.isEmpty())
        {
        	 return FuncUtils.getErrorMsg("0001",Configuration.getGlobalMsg("MSG_0001"));
        }
        int totalNum=new CatchOrderService().getOrderInfoListNum(lott);
